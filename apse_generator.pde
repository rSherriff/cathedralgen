...
{ 
  ...
  float apse_height = (vault_height * 2) + (vault_height * (number_of_apse_aisles - 1));
  int diff_between_choir = number_of_choir_aisles - number_of_apse_aisles;
  
  if(diff_between_choir > 1)
  {
    for(int i =1; i <= diff_between_choir; i++)
    {
      if(i % 2==0)
      {
        draw_outisde_wall( new PVector(origin.x, origin.y - (apse_height/ 2) - (vault_height/2)), pillar_width, vault_height, 1, 4, true);
      }
      else
      {
        draw_outisde_wall( new PVector(origin.x, origin.y + (apse_height/ 2) + (vault_height/2)), pillar_width, vault_height, 1, 4, true);
  ...
  draw_outisde_wall( new PVector(origin.x + (number_apse_vaults * vault_width), origin.y), pillar_width, apse_height, number_of_apse_aisles, 4, true);
